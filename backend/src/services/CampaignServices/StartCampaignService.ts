...
import SetPersonalMessage from "../../helpers/SetPersonalMessage";
...
const mountMessageData = (
  ...
  // Se tiver variáveis customizadas, usar a função de personalização
  if (campaign.customVariables && campaign.variablesData) {
    bodyMessage = SetPersonalMessage({ campaign, campaignContact });
  } else {
    // Sistema legado: selecionar mensagem aleatória entre as disponíveis
    const availableMessages: string[] = [];
    if (campaign.message1) availableMessages.push(campaign.message1);
    if (campaign.message2) availableMessages.push(campaign.message2);
    if (campaign.message3) availableMessages.push(campaign.message3);
    
    if (availableMessages.length > 0) {
      const randomIndex = randomInteger(0, availableMessages.length - 1);
      bodyMessage = availableMessages[randomIndex];
    } else {
      bodyMessage = campaign.message1; // Fallback
    }
  }
  ...
  return {
    ...
    messageRandom: campaign.customVariables ? "custom" : `message${randomInteger(1, 3)}`,
    campaignContact,
