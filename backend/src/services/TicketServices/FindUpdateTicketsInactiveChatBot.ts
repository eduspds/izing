...
const FindUpdateTicketsInactiveChatBot = async (): Promise<void> => {
  ...
  Promise.all(
    tickets.map(async (item: any) => {
      ...
      if (ticket) {
        // CORREÇÃO: Adicionar log detalhado para monitoramento
        const actionType = item.type_action == 1 ? "queue" : item.type_action == 2 ? "user" : "unknown";
        
        logger.info(
          `[FindUpdateTicketsInactiveChatBot] Processing inactive ticket | ticketId=${ticket.id} contactId=${ticket.contactId} status=${ticket.status} lastInteractionBot=${ticket.lastInteractionBot} actionType=${actionType} destiny=${item.destiny}`
        );
        ...
        logger.info(
          `[FindUpdateTicketsInactiveChatBot] Updated ticket due to inactivity | ticketId=${ticket.id} queueId=${values.queueId || 'unchanged'} userId=${values.userId || 'unchanged'}`
        );
