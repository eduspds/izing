...
import SendTestCampaignService from "../services/CampaignServices/SendTestCampaignService";
import ResetCampaignService from "../services/CampaignServices/ResetCampaignService";
...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  const schema = Yup.object().shape({
    ...
    message2: Yup.string().notRequired(),
    message3: Yup.string().notRequired(),
...
export const sendTestCampaign = async (
  ...
  const { campaignId, testNumber } = req.body;

  const schema = Yup.object().shape({
    campaignId: Yup.number().required(),
    testNumber: Yup.string().required().min(10).max(15)
  });

  try {
    await schema.validate({ campaignId, testNumber });
  } catch (error) {
    throw new AppError(error.message);
  }

  await SendTestCampaignService({
    campaignId,
    testNumber,
    tenantId
  });
...
export const resetCampaign = async (
  ...
  const result = await ResetCampaignService({
    campaignId,
    tenantId
  });
