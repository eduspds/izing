      ...
      >
          ...
          <q-card-section>
            <q-select
              ...
              v-model="item.evaluationChatFlowId"
              :options="listaChatFlow"
              map-options
              emit-value
              option-value="id"
            ...
            >
              <template v-slot:prepend>
                <q-icon name="mdi-star-check" />
              </template>
              <template v-slot:hint>
                Fluxo enviado após finalizar atendimento
              </template>
            </q-select>
          </q-card-section>
          <q-separator />
          ...
          >
            <template v-if="item.type !== 'messenger'">
              ...
              <q-btn
                ...
                color="orange"
                label="Reconectar"
                @click="handleReconnectWhatsApp(item.id)"
                :disable="!isAdmin || reconnectingSessions.includes(item.id)"
                :loading="reconnectingSessions.includes(item.id)"
                icon="refresh"
                class="q-mx-sm"
              >
                <q-tooltip>
                  Força uma nova conexão caso tenha caído
                </q-tooltip>
              </q-btn>
            ...
            <q-btn
              ...
              :disable="!isAdmin || !canDeleteConnection(item)"
              dense
              round
              flat
            ...
            >
              <q-tooltip>
                {{ canDeleteConnection(item) ? 'Deletar conexão' : 'Desconecte primeiro para poder deletar' }}
              </q-tooltip>
            </q-btn>
          </q-card-actions>
        </q-card>
...
import { DeletarWhatsapp, StartWhatsappSession, ListarWhatsapps, RequestNewQrCode, UpdateWhatsapp, ReconnectWhatsappSession, CloseWhatsappSession, VerificarMensagensPendentes } from 'src/service/sessoesWhatsapp'
...
export default {
  ...
  data () {
    return {
      ...
      whatsAppId: null,
      canais: [],
      reconnectingSessions: [], // Array para controlar sessões em reconexão
      objStatus: {
        qrcode: ''
      },
      columns: [
  ...
  methods: {
    ...
    canDeleteConnection (whatsapp) {
      if (!whatsapp) return false
      // Permite deletar quando não estiver conectado
    ...
    async handleDisconectWhatsSession (whatsAppId) {
      this.$q.dialog({
        ...
        this.loading = true
        CloseWhatsappSession(whatsAppId).then(() => {
    ...
    async handleReconnectWhatsApp (whatsAppId) {
      // Adicionar à lista de reconexão
      if (!this.reconnectingSessions.includes(whatsAppId)) {
        this.reconnectingSessions.push(whatsAppId)
      }

      try {
        // Usar o novo endpoint de reconexão inteligente
        const response = await ReconnectWhatsappSession(whatsAppId)
        ...
        if (response.data.status === 'CONNECTED') {
          this.$q.notify({
            ...
            progress: true,
            position: 'top',
            message: 'Sessão já estava conectada!',
            actions: [{
              icon: 'close',
              round: true,
              color: 'white'
        ...
        this.$q.notify({
          ...
          progress: true,
          position: 'top',
          message: 'Erro ao reconectar. Tente novamente.',
          actions: [{
            icon: 'close',
            round: true,
            color: 'white'
        ...
        const index = this.reconnectingSessions.indexOf(whatsAppId)
        if (index > -1) {
          this.reconnectingSessions.splice(index, 1)
        }
      }
    },
    ...
    async deleteWhatsapp (whatsapp) {
      if (!this.canDeleteConnection(whatsapp)) {
        this.$q.notify({
          type: 'warning',
          position: 'top',
          progress: true,
          message: 'Conexão ainda está ativa. Desconecte antes de deletar.',
          actions: [{
            icon: 'close',
            round: true,
            color: 'white'
      ...
      try {
        this.loading = true
        const { data } = await VerificarMensagensPendentes(whatsapp.id)
        const hasPendingMessages = data?.hasPendingMessages || false
        const messageCount = data?.count || 0

        let dialogMessage = 'Não é uma boa ideia apagar se já tiver gerado atendimentos para esse whatsapp.'
        if (hasPendingMessages) {
          dialogMessage = `Essa sessão tem ${messageCount} mensagem${messageCount > 1 ? 'ns' : ''} pendente${messageCount > 1 ? 's' : ''}. Tem certeza que quer apagar essa sessão?`
        }
        ...
        // Em caso de erro, mostrar modal padrão
        this.$q.dialog({
          title: 'Atenção!! Deseja realmente deletar? ',
