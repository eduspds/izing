...
import { UpdateWhatsapp, CriarWhatsapp, VerificarMensagensOrfas, TransferirMensagensOrfas } from 'src/service/sessoesWhatsapp'
      ...
      try {
        if (this.whatsAppEdit.id) {
          ...
          this.$q.notify({
            ...
            progress: true,
            position: 'top',
            message: 'Whatsapp editado com sucesso!',
            actions: [{
              icon: 'close',
              round: true,
              color: 'white'
          ...
          // Criar nova sessão
          const response = await CriarWhatsapp(whatsapp)
          const novaSessaoId = response?.data?.id
          ...
          // Verificar mensagens órfãs após criar nova sessão
          try {
            const { data: orphanedData } = await VerificarMensagensOrfas()
            const hasOrphanedMessages = orphanedData?.hasOrphanedMessages || false
            const orphanedCount = orphanedData?.count || 0

            if (hasOrphanedMessages && novaSessaoId) {
              // Mostrar modal perguntando se deseja transferir
              this.$q.dialog({
                title: 'Mensagens Desvinculadas Encontradas',
                message: `Existem ${orphanedCount} mensagem${orphanedCount > 1 ? 'ns' : ''} agendada${orphanedCount > 1 ? 's' : ''} desvinculada${orphanedCount > 1 ? 's' : ''} (sem vínculo de conexão). Deseja transferir para esta nova sessão?`,
                cancel: {
                  label: 'Não',
                  color: 'primary',
                  push: true
                ...
                try {
                  const { data: transferData } = await TransferirMensagensOrfas(novaSessaoId)
                  const transferred = transferData?.transferred || 0

                  this.$q.notify({
                    type: 'positive',
                    progress: true,
                    position: 'top',
                    message: `${transferred} mensagem${transferred > 1 ? 'ns' : ''} transferida${transferred > 1 ? 's' : ''} com sucesso!`,
                    actions: [{
                      icon: 'close',
                      round: true,
                      color: 'white'
                  ...
                  console.error('Erro ao transferir mensagens desvinculadas:', transferError)
                  this.$q.notify({
                    type: 'negative',
                    progress: true,
                    position: 'top',
                    message: 'Erro ao transferir mensagens desvinculadas.',
                    actions: [{
                      icon: 'close',
                      round: true,
                      color: 'white'
            ...
            console.error('Erro ao verificar mensagens desvinculadas:', orphanedError)
